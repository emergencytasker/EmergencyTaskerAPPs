...
using System;
using System.Text;
using EmergencyTask.API.Request;
...
namespace EmergencyTask.API
{
    public class Client
    {
...
#if DEBUG_
         static string BaseUrl = "http://192.168.1.113/emergencytasker/";
#else
         static string BaseUrl = "http://app.emergencytasker.com/";
#endif
        ...
        public static IApi<Calendar> Calendar
        {
        ...
        public static IApi<Time> Time
        {
            get { return new Api<Time>(Restclient); }
        }
        ...
        public static async Task<Response.Auth> Auth(string email, string password, string lenguaje)
        {
            var auth = await Restclient.ExecuteTaskAsync<Response.Auth>($"api/auth", RestSharp.Method.POST, new List<RestSharp.Parameter>
            {
                ...
                new RestSharp.Parameter("idperfil", (int)App.Perfil, RestSharp.ParameterType.GetOrPost),
                new RestSharp.Parameter("lenguaje", lenguaje, RestSharp.ParameterType.GetOrPost)
            });
            if (!auth.HasExecute) return new Response.Auth { code = -30, message = "Server Error", token = "", status = false, userid = 0 };
            return auth.Result ?? new Response.Auth { code = -40, message = "Message Error", token = "", status = false, userid = 0 };
        }
        ...
        public static async Task<UploadResponse> Upload(byte[] bytearray)
        {
            ...
            var uploadresponse = await Restclient.ExecuteTaskAsync<UploadResponse>($"api/upload", RestSharp.Method.POST, null, new List<File>
            {
                new File { Bytes = bytearray, FileName = "file.jpg", Name = "image" }
            });
        ...
        {
            var json = Newtonsoft.Json.JsonConvert.SerializeObject(new EmailVerified
            {
                id = id
            });
        ...
        public static async Task<bool> ChangeServiceStatus(int id, EstadoServicio estado, int idusuario, double latitud, double longitud, int tieneubicacion)
        {
            ...
            {
                { "idestadoservicio", estadodeservicio.ToString() }
            });
			Debug.WriteLine($"[ChangeServiceStatus] {requestservice}");
			var updated = requestservice != null && requestservice.idestadoservicio == (int)estado;
            Debug.WriteLine($"[ChangeServiceStatus] {updated}");
            if (updated)
            {
        ...
        /// <summary>
        /// Devuelve la distancia y tiempo de un punto gps a otro
        /// </summary>
        ...
        public static async Task<GoogleApi.Entities.Maps.DistanceMatrix.Response.Element> GoogleApiMatrixRequest(string key, double fromlat, double fromlng, double tolat, double tolng)
        {
            try
            {
                ...
                {
                    new LocationEx(new CoordinateEx(fromlat, fromlng))
                },
                    Destinations = new List<LocationEx>
                {
                    new LocationEx(new CoordinateEx(tolat, tolng))
                },
        ...
        /// <summary>
        /// Devuelve la ruta de un punto gps a otro punto gps
        /// </summary>
        /// <param name="fromlat"></param>
        /// <param name="fromlng"></param>
        /// <param name="tolat"></param>
        /// <param name="tolng"></param>
        /// <returns></returns>
        public static async Task<IList<Xamarin.Forms.GoogleMaps.Position>> GoogleMapsApiRoute(string key, double fromlat, double fromlng, double tolat, double tolng)
        {
            try
            {
                ...
                {
                    Origin = new LocationEx(new CoordinateEx(fromlat, fromlng)),
                    Destination = new LocationEx(new CoordinateEx(tolat, tolng)),
                    Language = GoogleApi.Entities.Common.Enums.Language.English,
                    Units = GoogleApi.Entities.Maps.Common.Enums.Units.Imperial,
            ...
            return new List<Xamarin.Forms.GoogleMaps.Position>(0);
        }
        ...
        public static async Task<Requestservice> GetCurrentService(Perfil perfil, int idusuario)
        {
            ...
            var now = date.Value;
			Debug.WriteLine($"[GetCurrentService] {perfil} | {idusuario}");
			...
			Debug.WriteLine($"[GetCurrentService] {servicesrequest}");
        ...
        /// </summary>
        /// <returns></returns>
        public static async Task<IEnumerable<Response.Service>> GetServices(string lang)
        {
            var response = await Restclient.ExecuteTaskAsync<IEnumerable<Response.Service>>($"api/getservices?lang={lang}", RestSharp.Method.GET);
        ...
        /// <param name="message"></param>
        /// <returns></returns>
        public static async Task<Response.SendNotification> SendNotification(int idusuario, string title, string message, int data, int action)
        {
            var response = await Restclient.ExecuteTaskAsync<Response.SendNotification>("api/sendnotification", RestSharp.Method.POST, new List<RestSharp.Parameter>
            {
                new RestSharp.Parameter("idusuario", idusuario, RestSharp.ParameterType.GetOrPost),
        ...
        public static async Task<IEnumerable<string>> GetAddressByPoint(string key, double latitud, double longitud)
        {
            try
            {
                ...
                {
                    Location = new CoordinateEx(latitud, longitud),
                    Language = GoogleApi.Entities.Common.Enums.Language.English,
                    Key = key
                });
                if (algo == null) return null;
        ...
        public static async Task<IEnumerable<Response.Tasker>> ListTaskersByAvailability(double latitud, double longitud, int idcategoria, int idsubcategoria, 
            DateTime fecha, TimeSpan time)
        {
            var response = await Restclient.ExecuteTaskAsync<IEnumerable<Response.Tasker>>("api/listtaskers", RestSharp.Method.GET, new List<RestSharp.Parameter>
            {
            ...
            return response.HasExecute ? (response.Result ?? new List<Response.Tasker>()) : new List<Response.Tasker>();
        }
