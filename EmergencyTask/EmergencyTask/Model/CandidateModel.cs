...
namespace EmergencyTask.Model
{
    public class CandidateModel : ModelBase
    {
        ...
        public CandidateModel(API.Response.Tasker tasker)
        {
            ...
            CostoTime = $"${Math.Round(tasker.Costoporhora, 2)} {AppResource.PorHora}";
            Review = tasker.Calificacion;
            Task = $"{tasker.Tareascompletas} {AppResource.Tareas}";
        ...
        public CandidateModel(User user, Service service)
        {
            ...
            Cost = service.costo;
            CostoTime = $"${Math.Round(service.costo, 2)} {AppResource.PorHora}";
            Review = user.calificacion;
            Task = $"{user.tareas} {AppResource.Tareas}";
            Distancia = 0;
